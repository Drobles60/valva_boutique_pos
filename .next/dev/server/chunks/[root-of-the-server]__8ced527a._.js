...
"[project]/app/api/reportes/inventario/bajo-stock/route.ts [app-route] (ecmascript)", ((__turbopack_context__) => {
"use strict";
...
async function GET(request) {
    try {
      ...
      ORDER BY p.stock_actual ASC, p.nombre ASC`, [
        ...
        const ultimasVentas = await (0, __TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$db$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["query"])(`SELECT 
        dv.producto_id,
      ...
      GROUP BY dv.producto_id`);
        ...
        const productosData = productos.map((p)=>{
            ...
            return {
                ...
                valor_reposicion: stock > 0 ? 0 : Number(p.precio_compra) * 5,
                ultima_venta: ventasMap[p.producto_id]?.ultima_venta || null,
                vendidos_total: ventasMap[p.producto_id]?.vendidos_total || 0,
                nivel
            };
        });
        const agotados = productosData.filter((p)=>p.stock_actual === 0).length;
        const criticos = productosData.filter((p)=>p.nivel === "Crítico").length;
        const bajos = productosData.filter((p)=>p.nivel === "Bajo").length;
        // Total active products for percentage
        const [totalRow] = await (0, __TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$db$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["query"])(`SELECT COUNT(*) AS total FROM productos WHERE estado = 'activo'`);
...
//# sourceMappingURL=%5Broot-of-the-server%5D__8ced527a._.js.map